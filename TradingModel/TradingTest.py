...
class Investor:
	def __init__(self,_name = 'ZZZZ', _intial_virtual_shares = 1000, _start_date = '2010-06-04', _stockcode = 600050, _interval = 10):
		self.name = _name
		self.now = 0
		...
		_intial_cash = self.dp.getPrice(self.stocks,0)*_intial_virtual_shares
	...
	def LongOneShare(self,which):
		nowPrice = self.dp.getPrice(stock=self.stocks, date_count=self.now)
	...
	def SellAndShortOne(self,which):
		...
		tomoPrice[which] = self.dp.getPrice(stock=self.stocks, date_count=self.now+1)
	...
	def TradeNext(self):
		...
		trendReal = int (self.dp.getPrice(stock=self.stocks, date_count=self.now+self.interval) >= self.dp.getPrice(stock=self.stocks, date_count=self.now))

		# print "price:",self.dp.getPrice(stock=self.stocks, date_count=self.now+self.interval) ,self.dp.getPrice(stock=self.stocks, date_count=self.now)
	...
	def getTotalROR(self):
		...
		return ttlROR
def main():
	ZZZZ = Investor(_name = 'ZZZZ', _intial_virtual_shares = 100, _start_date = '2014-06-04', _stockcode = 600050, _interval = 7)
